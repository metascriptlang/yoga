...
- **Local additions (NOT upstream)**: `build/` (macOS and Emscripten `libyoga.a` archives produced by `scripts/build-yoga.sh`) and this file. iOS compiles the vendored sources declared in `src/yogaH.ms`. Never overwrite the local additions when re-vendoring.
